<?php
/**
 * Save the title Umpire shows when displaying and mentioning an entry form field.
 *
 * @category Administrative
 * @package  Umpire
 * @author   A.E.Veltstra for OmegaJunior Consultancy <omegajunior@protonmail.com>
 * @version  2.24.929.1842
 */
...
$old_translation_from_post = '';
if (isset($_POST['old_translation'])) {
    $old_translation_from_post = $_POST['old_translation'];
}

$new_translation_from_post = '';
if (isset($_POST['new_translation'])) {
    $new_translation_from_post = $_POST['new_translation'];
}
...
if (!empty($field_choice)) {
    ...
    if ($is_existing_record_found) {
        $existing_record_has_old_value = isset(
            $get_existing_record[0]['translation']
        );
        if ($existing_record_has_old_value) {
            $old_value_from_record = $get_existing_record[0]['translation'];
            $old_values_match = (
                $old_value_from_record == $old_translation_from_post
            );
            if ($old_values_match) {
                try {
                    $result = db_exec(
                ...
                echo '{
                  ...
                  "errors": [
                    ...
                    "Maybe someone else changed the translation already.",
        ...
        try {
            $result = db_exec(
                'insert into `attribute_translations` 
                    (`translation`, `language_code`, `attribute_id`)
                    values 
                    (?, ?, ?)
                ',
                'sss',
